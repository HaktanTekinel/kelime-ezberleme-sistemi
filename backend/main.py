...
def register_user(user: schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    # Yeni kullanıcı oluştur (Şifre hash'leme gerçek projede şarttır)
...
def forgot_password(payload: schemas.PasswordUpdate, db: Session = Depends(get_db)):
    ...
    return {"message": "Şifre güncellendi", "user_id": db_user.id}
...
def create_word(word_data: schemas.WordCreate, db: Session = Depends(get_db)):
    ...
    # 2. Örnek cümleleri oluştur
    for sample in word_data.samples:
        new_sample = models.WordSample(word_id=new_word.id, sample_text=sample)
        db.add(new_sample)
