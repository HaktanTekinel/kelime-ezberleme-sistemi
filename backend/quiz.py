...
def get_daily_quiz(user_id: int, db: Session = Depends(get_db)):
    ...
    # Öğrenilmemiş (is_learned == False) ve zamanı gelmiş kelimeleri getir
    due_progress_items = (
        ...
        .filter(models.UserWordProgress.is_learned == False) # EKLENEN FİLTRE
...
def submit_quiz_answer(payload: schemas.QuizAnswerRequest, db: Session = Depends(get_db)):
    now = datetime.utcnow()
    ...
    progress = (
        ...
        .filter(models.UserWordProgress.user_id == payload.user_id)
        .filter(models.UserWordProgress.word_id == payload.word_id)
        .first()
    )
    if not progress:
        progress = models.UserWordProgress(user_id=payload.user_id, word_id=payload.word_id)
        db.add(progress)
        db.flush()
    ...
    if is_correct:
        # Girinti (indentation) düzeltildi
